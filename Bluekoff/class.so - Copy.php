...
class sod
{
    function def()
    {
        ...
        <div class='container'>
            <div class='row'>
				...
				if ($acl > '3') {
				?>
                <input type='button' value='Print' onclick='window.open("point.php?cat=so&typ=all","_self")'>
                <?php
				}
				?>
    ...
    function edit()
    {
        ...
        {
            $head = "Add";
            $sql = "select * from quo where id = '".$_REQUEST['ref']."'";
            $ref = $_REQUEST['ref'];
            $res = $conn->query($sql);
            while ($cdr = $res->fetch()) 
            {
        ...
        <div class='container'>
            <div class='row'>
                ...
                <table class='table table-bordered table-striped'>
                    ...
                    $sql = "select `inventory`.`name` as name
                    , ifnull((
					select `so_detail`.`cost` from `so_detail` where `so_detail`.`status` = '1' and `inventory`.`id` = `so_detail`.`inventory_id` and `so_detail`.`so_id` = '".$id."'
					) , `inventory`.`buy`) as so
                    , `inventory`.`id` as stid
                    , (
    ...
    function save()
    {
        ...
        {
            $sql = "insert into so set customer_id = '".$customer."', date = '".$date."', `uid` = '".$_SESSION['uid']."', `quo_id` = '".$ref."'";
        ...
        while ($a < $limit) 
        {
            if ($_REQUEST['num-'.$a] > 0) 
            {
                $sql = "insert into so_detail set so_id = '".$id."', inventory_id = '".$_REQUEST['id-'.$a]."', num = '".$_REQUEST['num-'.$a]."', cost = '".$_REQUEST['cost-'.$a]."'";
                $conn->query($sql);
            }
            $a++;
        }
                    ...
                    <tr>
                        <td colspan='2' class='text-center'>
							<?php
								...
								{
									?>
									<input type="button" value="print" onclick="window.open('point.php?cat=so&typ=det&id=<?php echo $id;?>','_self')">
									<input type="button" value="Back" onclick="window.open('index.php?option=so&task=def','_self')">
									<?php
								}
								else
